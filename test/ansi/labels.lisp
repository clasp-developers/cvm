...
(5am:def-suite labels :in data-and-control-flow)
(5am:in-suite labels)
...
(5am:test labels.39
  (is-true-eval (labels ((f () #'f))
                  (eql (f) #'f))))

(5am:test labels.40
  (is-true-eval (let ((x (labels ((f () #'f)) #'f)))
                  (eql x (funcall x)))))
