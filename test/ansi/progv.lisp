...
(deftest progv.10
  (let ((x 0) (y 0) (c 0))
    (progv
        (progn (s:setf x (s:incf c)) nil)
        (progn (s:setf y (s:incf c)) nil)
      (values x y c)))
  1 2 2)
...
(deftest progv.13
  (macrolet
   ((%m (z) z))
   (progv (s:expand-in-current-env (%m '(x)))
          '(:good)
...
(deftest progv.14
  (macrolet
   ((%m (z) z))
   (progv (list (s:expand-in-current-env (%m 'x)))
          '(:good)
          (locally (declare (special x)) x)))
  :good)
...
(deftest progv.16
  (macrolet
   ((%m (z) z))
   (progv '(x)
          (list (s:expand-in-current-env (%m :good)))
          (locally (declare (special x)) x)))
  :good)
...
(deftest progv.17
  (macrolet
   ((%m (z) z))
   (progv nil nil (s:expand-in-current-env (%m :good))))
  :good)
