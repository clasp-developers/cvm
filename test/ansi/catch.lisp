...
#+(or) ; assert is weird/not defined in cross
(deftest catch.9
  (let ((i 0))
...
(deftest catch.14
  (macrolet ((%m (z) z))
    (catch 'foo (s:expand-in-current-env (%m :good))))
  :good)

(deftest catch.15
  (macrolet ((%m (z) z))
    (catch 'foo (throw (s:expand-in-current-env (%m 'foo)) :good) :bad))
  :good)

(deftest catch.16
  (macrolet ((%m (z) z))
    (catch 'foo (throw 'foo (s:expand-in-current-env (%m :good))) :bad))
  :good)

(5am:test throw-error
  (signals-eval control-error (throw (list nil) nil)))
